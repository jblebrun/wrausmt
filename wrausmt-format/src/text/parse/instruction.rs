use {
    ...
    crate::text::{num, parse::combinator::Groups, token::Token},
    std::io::Read,
...
impl<R: Read> Parser<R> {
    pub fn parse_instructions(&mut self) -> Result<Vec<Instruction<Unresolved>>> {
        ...
        Ok(self.zero_or_more_groups(Self::try_instruction)?.result)
    }
        ...
        match instruction_data {
            Some(data) => {
                ...
                let operands = match data.operands {
                    ...
                    Operands::Select => {
                        let Groups { result, count } =
                            self.zero_or_more_groups(Self::try_parse_fresult)?;
                        if count == 0 {
                            syntax::Operands::None
                        } else {
                            syntax::Operands::SelectT(result)
                        }
                    }
                    Operands::SelectT => {
    ...
    fn parse_folded_if(&mut self) -> Result<Vec<Instruction<Unresolved>>> {
        ...
        let thexpr = if self.try_expr_start("then")? {
            let instr = self.zero_or_more_groups(Self::try_instruction)?.result;
            self.expect_close()?;
            UncompiledExpr { instr }
        } else {
