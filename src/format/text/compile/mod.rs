...
fn emitter32(body: &mut Vec<u8>, v: u32) {
    let bytes = &v.to_le_bytes()[..];
    body.extend(bytes);
}

fn emitter64(body: &mut Vec<u8>, v: u64) {
    let bytes = &v.to_le_bytes()[..];
    body.extend(bytes);
}
    ...
    let functype = match &func.typeuse.typeidx {
        ...
        None => {
            ...
            types.iter()
                ...
                .map(|p| p as u32)
                .unwrap_or_else(|| panic!("inline type missing from type list. this is a compiler error."))
        }
    };
