...
pub trait ReadImports : ReadWasmValues {
        ...
        (0..items).map(|_| {
            Ok(Import {
                ...
                desc: {
                    ...
                    match kind {
                        0 => ImportDesc::Func(self.read_leb_128().wrap("parsing func")?),
    ...
    fn read_limits(&mut self) -> Result<Limits> {
        ...
        Ok(Limits {
            lower: self.read_leb_128().wrap("parsing lower")?,
            upper: if has_upper { Some(self.read_leb_128().wrap("parsing upper")?) } else { None }
        })
    }
}
